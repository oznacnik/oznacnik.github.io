...
export default async function HomePage() {
  ...
  // Anonymizace pro veřejnost (web_consent = false → "Anonym N")
  const anonAuthors = authorsWithProgress.filter((a) => !a.web_consent);
  let anonIdx = 0;
  const displayAuthors = authorsWithProgress.map((a) => {
    if (a.web_consent) {
      return { ...a, publicName: displayName(a), publicAnnotation: a.annotation };
    }
    anonIdx++;
    return {
      ...a,
      publicName: anonAuthors.length === 1 ? "Anonym" : `Anonym ${anonIdx}`,
      publicAnnotation: null as string | null,
    };
  });
  ...
  return (
    <div>
                      ...
                      return (
                        ...
                        >
                          {realWorks.length === 0 ? (
                            ...
                            >
                              {claimedWorks.length === 0 ? "Díla nezadána" : "Bez sdělení"}
                            </div>
                          ) : (
      ...
      <div className="px-6 py-6 flex justify-between items-center">
        <Link href="/archiv" className="nav-link">Archiv výstav →</Link>
